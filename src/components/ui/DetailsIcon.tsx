export default function DetailsIcon() {
    return (
        <svg fill="#000000" viewBox="0 0 36 36" height='1em' width='1em' version="1.1" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
            <title>details-line</title>
            <path d="M32,6H4A2,2,0,0,0,2,8V28a2,2,0,0,0,2,2H32a2,2,0,0,0,2-2V8A2,2,0,0,0,32,6Zm0,22H4V8H32Z">
            </path><path d="M9,14H27a1,1,0,0,0,0-2H9a1,1,0,0,0,0,2Z">
            </path>
            <path d="M9,18H27a1,1,0,0,0,0-2H9a1,1,0,0,0,0,2Z">
            </path><path d="M9,22H19a1,1,0,0,0,0-2H9a1,1,0,0,0,0,2Z">
            </path>
            <rect x="0" y="0" width="36" height="36" fill-opacity="0" />
        </svg>
    )
}